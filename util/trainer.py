...
def validate(val_loader, model, training_dataset,epoch, check_mode,device,Yscaler,half=0):
    ...
    for i, (x, y_gt) in enumerate(val_loader):
        ...
        if training_dataset=="MCD":
            y_pred_unscaled = Yscaler.inverse_transform(y_pred.data.cpu())
            y_gt_unscaled   = Yscaler.inverse_transform(y_gt.data.cpu())
        else:
...
def network_train(model,training_dataset, train_dataloader, test_dataloader, Yscaler, criterion, optimizer, num_epoch, device, lr, decay_rate, decay_step, vis=1, save_weights=0, save_path='weights', log_path='logs/',half=0,real=0):
    best_res = 1000
    ...
    for epoch in range(num_epoch):
        ...
        res = validate(test_dataloader, model, training_dataset, epoch, 'Validation', device, Yscaler,half=half)  # evaluate on test set
